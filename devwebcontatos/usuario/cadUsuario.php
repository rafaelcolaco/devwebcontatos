        ...
        <div class='col'>
            <form action="acao.php" method="post">  <!-- esse formulário envia os dados para o arquivo acao.php -->
                <div class='row'>
                    ...
                    <div class='col'>
                        <label for="name">Passatempo:</label>
                        <input type="text" class='form-control' name='passatempo' id='passatempo' placeholder="Informe seu passatempo..." value=<?php if(isset($usuario)) echo $usuario['passatempo']?>>
                    </div>
                    <div class='col'>  
                        <br>                  
                    ...
                    <?php             
                        try{
                            ...
                            foreach($usuarios as $usuario){ // percorre o array com todos os usuários imprimindo as linhas da tabela
                                ...
                                echo '<tr><td>'.$usuario['id'].'</td><td>'.$usuario['nome'].'</td><td>'.$usuario['email'].'</td><td>'.$usuario['senha'].'</td><td>'.$usuario['cidade'].'</td><td>'.$usuario['passatempo'] .'</td><td>'.$editar.'</td><td>'.$excluir.'</td></tr>';
                            }
                        }catch(PDOException $e){ // se ocorrer algum erro na execuçao da conexão com o banco executará o bloco abaixo
                            print("Erro ao conectar com o banco de dados...<br>".$e->getMessage());
                            die();
